...
class Importer extends Singleton {
	...
	public function __construct( $menus = [] ) {
		$this->menus = $menus;
		add_action('import_start', [ $this, 'allow_custom_upload' ]);
	...
	public function process_customizer(): void {
		...
		if ( $theme_mod ) {
			...
			$remapped      = json_decode(get_transient('rbb_remapped_terms') ?? [], true, 512, JSON_THROW_ON_ERROR);
	 ...
	 * @return void
	 */
	public function allow_custom_upload(): void {
		add_filter(
			'upload_mimes',
			function ( $mimes ) {
