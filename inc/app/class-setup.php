...
class ThemeSetup {
	...
	public function __construct() {
		...
		add_filter('pt-importer/time_for_one_ajax_call', [ $this, 'time_ajax_call' ]);
	...
	public function update_permalink_structure( $selected_import_index ): void {
		...
		if ( RisingBambooCoreHelper::woocommerce_activated() ) {
	...
	public function update_comment_count(): void {
		...
		foreach ($entries as $entry) {
			...
			$this->merlin->logger->info(__('Update for ID : '. $post_id, 'botanica'), [$result]);
		}
	...
	public function update_woocommerce_pages( $selected_import_index ): void {
		...
		if ( ! empty($selected_import_data) && RisingBambooCoreHelper::woocommerce_activated() && strpos($selected_import_data['import_file_url'], 'extra.xml') !== false ) {
	...
	public function fix_missing_pages(): void {
		if (
			RisingBambooCoreHelper::woocommerce_activated() &&
			...
			remove_filter('woocommerce_create_pages', [ $this, 'prevent_create_pages' ]);
