...
class Setting extends Singleton {
	...
	protected function get_option( string $key, $meta_key = null ) {
		...
		return get_post_meta(self::get_id(), ( ! empty($meta_key) ? $meta_key : $key ), true) ?: RisingBambooKirki::get_option(RISING_BAMBOO_KIRKI_CONFIG, $key); // phpcs:ignore WordPress.PHP.DisallowShortTernary.Found
	}
	...
	protected function set_override_data(): void {
		if ( empty(self::$override_data) && RbbCoreSettingHelper::get_option('development_override_settings') ) {
