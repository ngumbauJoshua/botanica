...
class Merlin {
	...
	public function admin_page(): void {
		...
		// Enqueue styles.
		wp_enqueue_style( 'merlin', RBB_THEME_DIST_URI . '/css/admin/setup.css', ['wp-admin'], MERLIN_VERSION );

		// Enqueue javascript.
		wp_enqueue_script( 'merlin', RBB_THEME_DIST_URI . '/js/admin/setup.js', ['jquery-core'], MERLIN_VERSION );

		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'botanica' ),
		);
	 ...
	 * SVG
	 */
	public function svg_sprite(): void {
	 ...
	 */
	public function svg(array $args = array() ): string {

		// Make sure $args are an array.
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'botanica' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'botanica' );
		}
	 ...
	 */
	public function loading_spinner(): string {
		ob_start();
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'botanica' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'botanica' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'botanica' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'botanica' ),
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'botanica' ) );
	}
	...
	protected function license() {
			...
			<?php if ( ! $is_theme_registered ) : ?>
				<div class="merlin__content--license-key">
					...
					<div class="merlin__content--license-key-wrapper">
						<input type="text" id="license-key" class="js-license-key" autocomplete="off" autocapitalize="off" spellcheck="false">
						<?php if ( ! empty( $action_url ) ) : ?>
							<a href="<?php echo esc_url( $action_url ); ?>" target="_blank">
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'botanica' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'botanica' ) );
	}
	...
	protected function plugins() {
		...
		$credentials  = request_filesystem_credentials( esc_url_raw( $url ), $method, false, false, $fields );
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'botanica' ); ?>">
										<?php esc_html_e( 'req', 'botanica' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'botanica' ) );
	}
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'botanica' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'botanica' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'botanica' ) );
	}
	 ...
	 */
	public function generate_child(): void {
        if(check_ajax_referer( 'merlin_nonce', 'wpnonce' )) {
	        ...
	        if ( ! file_exists( $path ) ) {
		        ...
		        $this->logger->debug( __( 'The existing child theme was activated', 'botanica' ) );
	        ...
	        $this->logger->debug( __( 'The newly generated child theme was activated', 'botanica' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'botanica' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'botanica' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'botanica' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'botanica' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'botanica' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'botanica' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'botanica' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'botanica' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'botanica' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'botanica' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'botanica' ) );
	 ...
	 */
	public function generate_child_style_css(string $slug, string $parent, string $author, string $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'botanica' ) );
	 ...
	 */
	public function generate_child_screenshot( $path ): void
	{
		...
		} else if ( file_exists( $this->base_path . '/screenshot.png' ) ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'botanica' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'botanica' ), array( 'screenshot' => $screenshot ) );
		}
	}
	 ...
	 */
	public function _ajax_plugins(): void
	{

		if ( empty( $_POST['slug'] ) || ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			exit( 0 );
		}
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'botanica' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'botanica' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'botanica' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'botanica' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'botanica' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'botanica' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'botanica' ),
				)
			);
		}
	 ...
	 */
	public function _ajax_content(): void {
		static $content = null;

		$selected_import = (int)$_POST['selected_index'];

		if ( null === $content ) {
			$content = $this->get_import_data( $selected_import );
		}
		if ( (empty($_POST['content']) && isset($content[$_POST['content']])) || ! check_ajax_referer( 'merlin_nonce' )) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'botanica' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'botanica' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'botanica' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'botanica' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'botanica' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	 ...
	 */
	public function _ajax_get_total_content_import_items(): void
	{
		if ( empty( $_POST['selected_index'] ) && ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'botanica' ) );
			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'botanica' ),
				)
			);
		}
	 ...
	 * @return bool|array
	 */
	public function get_import_data_info(int $selected_import_index = 0 ) {
		$import_data = array(
			'content'      => false,
			'widgets'      => false,
			'options'      => false,
	 ...
	 * @return    array
	 */
	protected function get_import_data(int $selected_import_index = 0 ): array {
		$content = array();
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'botanica' ),
				'description'      => esc_html__( 'Demo content data.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'botanica' ),
				'description'      => esc_html__( 'Sample widgets data.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'botanica' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'botanica' ),
				'description'      => esc_html__( 'Sample theme options data.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'botanica' ),
				'description'      => esc_html__( 'Redux framework options.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'botanica' ),
				'description'      => esc_html__( 'After import setup.', 'botanica' ),
				'pending'          => esc_html__( 'Pending', 'botanica' ),
				'installing'       => esc_html__( 'Installing', 'botanica' ),
				'success'          => esc_html__( 'Success', 'botanica' ),
		...
		return apply_filters( 'merlin_get_base_content', $content, $this );
	}
	 ...
	 */
	public function import_revolution_sliders(string $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'botanica' ) );
	 ...
	 */
	public function pt_importer_new_ajax_request_response_data( $data ): array {
		$data['url']      = admin_url( 'admin-ajax.php' );
		$data['message']  = esc_html__( 'Installing', 'botanica' );
		$data['proceed']  = 'true';
		$data['action']   = 'merlin_content';
		$data['content']  = 'content';
		$data['_wpnonce'] = wp_create_nonce( 'merlin_nonce' );
		$data['hash']     = md5( mt_rand() ); // Has to be unique (check JS code catching this AJAX response).
	 ...
	 */
	public function before_content_import_setup(): void {
		...
		if ( ! empty( $posts ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'botanica' ) );
		}
	}
	 ...
	 */
	public function register_import_files(): void
	{
	 ...
	 */
	public function validate_import_file_info(array $import_files ): array
	{
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'botanica' ), $import_file );
			}
		}
	 ...
	 */
	public function set_import_file_base_name(): void
	{
	 ...
	 * @return array
	 */
	public function get_import_files_paths( $selected_import_index ): array
	{
	 ...
	 */
	public function update_selected_import_data_info(): void
	{
        if(check_ajax_referer( 'merlin_nonce', 'wpnonce' )) {
	        $selected_index = ! isset( $_POST['selected_index'] ) ? false : (int)$_POST['selected_index'];
	 ...
	 * @return string
	 */
	public function get_import_steps_html( $import_info ): string
	{
		ob_start();
		?>
	 ...
	 */
	public function import_finished(): void {
        if(check_ajax_referer( 'merlin_nonce', 'wpnonce' )) {
