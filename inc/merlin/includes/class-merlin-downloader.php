...
class Merlin_Downloader {
	 ...
	 * @var string
	 */
	private string $download_directory_path = '';
	 ...
	 */
	public function __construct(string $download_directory_path = '' ) {
	 ...
	 */
	public function download_file(string $url, string $filename ) {
		...
		Merlin_Logger::get_instance()->error(
			__('The file was not able to save to disk, while trying to download it', 'botanica'),
			[
				'url'      => $url,
				'filename' => $filename,
			]
	 ...
	 */
	private function get_content_from_url(string $url ) {
		// Test if the URL to the file is defined.
		if ( empty($url) ) {
			return new \WP_Error(
				'missing_url',
				__('Missing URL for downloading a file!', 'botanica')
			);
		}
		...
		$response = wp_remote_get(
			$url,
			[ 'timeout' => apply_filters('merlin_timeout_for_downloading_import_file', 20) ]
		);
		...
		if ( is_wp_error($response) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__('An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'botanica'),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
	 ...
	 */
	private function get_error_from_response( $response ): array
	{
		$response_error = [];
	 ...
	 */
	public function get_download_directory_path(): string
	{
	 ...
	 */
	public function set_download_directory_path(string $download_directory_path ): void
	{
	 ...
	 * @return bool|string
	 */
	public function fetch_existing_file(string $filename ) {
