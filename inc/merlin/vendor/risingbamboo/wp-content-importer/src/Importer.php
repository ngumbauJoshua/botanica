...
class Importer extends WXRImporter
{
	...
	public function import(string $file, $opts = []): bool
	{
		...
		add_filter( 'upload_mimes', [$this, 'allow_custom_file_upload']);
	...
	public function new_ajax_request_maybe(array $data): array
	{
		...
		if ($time > apply_filters('pt-importer/time_for_one_ajax_call', 20)) {
			$response = apply_filters('pt-importer/new_ajax_request_response_data', [
				'status' => 'newAJAX',
	...
	{
		$data = apply_filters('pt-importer/set_current_importer_data', [
	...
	{
		$flag = set_transient('pt_importer_data', $data, MINUTE_IN_SECONDS);
