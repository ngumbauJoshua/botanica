...
?>
<div id="rbb-promotion-popup" class="rbb-promotion-popup hidden <?php echo ( false === $outside ) ? 'outside-modal' : ''; ?> <?php echo esc_attr($promotion_popup); ?>" data-modal-animation="<?php echo esc_attr($modal_effect); ?>" data-modal = '{
...
<div class="rbb-promotion-popup-inner flex rounded-[18px] overflow-hidden">
	...
	<div class="popup-content md:w-[52.2%] w-full lg:px-[65px] lg:pt-20 lg:pb-5 md:px-10 md:pt-10 p-6 text-center relative bg-white z-10 rounded-r-[18px]">
		...
		<?php } ?>
		<?php if ( $args['title'] ) { ?>
			<h2 class="title text-[24px] font-extrabold uppercase pb-4 text-[color:var(--rbb-title-color)]"><?php echo esc_html($args['title']); ?></h2>
		<?php } ?>
		<?php if ( $args['desc'] ) { ?>
