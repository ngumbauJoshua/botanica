...
<form role="search" method="get" class="<?php echo esc_attr($class_form); ?>" action="<?php echo esc_url(home_url('/')); ?>" data-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-noresult="<?php echo esc_html__('No Result', 'botanica'); ?>" data-limit="<?php echo esc_attr($limit); ?>">
	<div class="relative input-group flex bg-white rounded-[--rbb-search-input-border-radius]">
		...
		<div class="relative w-full">
			<?php if ( in_array($header, [ 'header-07', 'header-08' ], true) ) { ?>
			...
			<label class="whitespace-nowrap md:block hidden duration-300 pointer-events-none text-[13px] text-[#a0a0a0] absolute top-[51%] -translate-y-1/2 left-6"><?php echo esc_attr__('Enter Your Keyword', 'botanica'); ?></label>
			<button class="button-icon btn-search absolute flex items-center justify-center w-[70px] h-[52px] text-center md:-right-[2px] right-0 -top-[2px] duration-300" type="submit">
				...
				<span class="text-[0px]"><?php echo esc_html__('search', 'botanica'); ?></span>
			</button>
