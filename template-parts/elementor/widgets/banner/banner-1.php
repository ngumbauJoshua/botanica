...
<div id="<?php echo esc_attr($id); ?>" class="relative group overflow-hidden rounded-[20px] <?php echo esc_attr($layout); ?>">
	<div class="lg:grid relative">
		<div class="lg:mb-0 mb-[30px] mx-auto md:max-w-[460px] lg:absolute right-0 z-10 text-center lg:top-[58%] lg:-translate-y-1/2 md:px-10 px-[15px]">
			<div class="">
				<div class="title_block">
					<p class="sub_title text-5xl"><?php echo wp_kses_post($sub_title); ?></p>
					<h4 class="title text-5xl"><?php echo wp_kses_post($title); ?></h4>
				</div>
				<?php
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img">
							...
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="scale-100 duration-300 group-hover:scale-105 <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
