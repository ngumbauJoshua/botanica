...
<div id="<?php echo esc_attr($id); ?>" class=" relative z-10 <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="grid grid-cols-12 gap-0">
			<div class="banner-left rounded-[10px] overflow-hidden group col-span-12 lg:col-span-6">
				...
				foreach ( $images as $image ) {
					...
					?>
						...
						<?php } ?>
						<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="<?php echo esc_attr(( $parallax ) ? 'layer' : ' scale-100 duration-300 group-hover:scale-105'); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
						<?php if ( $parallax ) { ?>
			...
			<div class="col-span-12 lg:col-span-6 relative xl:pl-[95px] lg:pl-10 lg:max-w-[800px]">
				<div class="lg:text-left text-center xl:pt-[100px] lg:pt-5 pt-10">
					<div class="title_block">
						<p class="sub_title wow fadeInUp"><?php echo wp_kses_post($sub_title); ?></p>
						<h2 class="title text-2xl xl:leading-[55px] leading-9 wow fadeInUp"><?php echo wp_kses_post($title); ?></h2>
					</div>
					<?php
					...
					?>
					<div class="desc text-base mt-6 xl:pb-5 pb-3 leading-6 wow slideInUp"><?php echo wp_kses_post($description); ?></div>
					<div class="mt-2 wow slideInUp">
						<?php
						foreach ( $custom_html as $html ) {
							...
							echo "<div class='custom-html pb-3 text-center min-w-20 md:pt-0 pt-5 md:w-auto w-1/2 inline-block " . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
						}
						?>
					</div>
				<div class="pt-6 wow slideInUp">
					<?php
					foreach ( $buttons as $button ) {
						...
						<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
							...
							class="inline-block btn h-[58px] leading-[58px] pl-8 pr-[7px] rounded-[58px] rbb-button <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
