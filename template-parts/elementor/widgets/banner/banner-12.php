...
<div id="<?php echo esc_attr($id); ?>" class="relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="lg:grid relative group">
		<div class="inset-x-0 top-1/2 -translate-y-1/2 absolute scale-[0.3] opacity-0 group-hover:scale-100 group-hover:opacity-100 duration-300 z-10 text-center p-[15px] flex justify-center items-center">
			<div class="text-white">
				<div class="title_block font-bold">
					<h4 class="title text-2xl"><?php echo wp_kses_post($title); ?></h4>
					<p class="sub_title mb-0"><?php echo wp_kses_post($sub_title); ?></p>
				</div>
				<?php 
				if ( ! empty($description) ) {
					?>
					<div class="desc text-sm mt-4 leading-[23px]"><?php echo wp_kses_post($description); ?></div><?php } ?>
			</div>
			<?php
			if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
				?>
				<div class="py-2 banner-countdown -mx-[5px]">
			...
			?>
			<div class="flex items-left justify-left mt-[5px]">
				<?php
				foreach ( $buttons as $button ) {
					...
					<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
						...
						class="inline-block btn text-white<?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
						<?php
					...
					>
						...
						<?php } ?>
						<span class="button-text flex items-center"><?php echo wp_kses_post($button_name); ?>
						<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
							...
							if ( ! empty($button_icon['value']) ) {
								...
								?>
								<i class="rbb-icon-direction-711 pl-2 text-xl" aria-hidden="true"></i>
							<?php } ?>
						<?php } ?>
						</span>
					</a>
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img">
							...
							<?php } ?>
							<div class="inset-0 absolute scale-[0.3] opacity-0 group-hover:scale-100 group-hover:opacity-100 duration-300 bg-black/50 z-[2]"></div>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="<?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block w-full <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
