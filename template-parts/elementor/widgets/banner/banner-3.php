...
<div id="<?php echo esc_attr($id); ?>" class=" relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="container mx-auto overflow-hidden pl-[10px] rounded-[18px]">
		<div class="grid grid-cols-12 gap-4 min-h-[360px]">
			<div class="col-span-12 md:col-span-6 relative pt-16">
				<div class="md:text-left text-center">
					<div class="title_block">
						<p class="sub_title mb-[3px]"><?php echo wp_kses_post($sub_title); ?></p>
						<h4 class="title xl:text-4xl text-2xl xl:!leading-[50px] !leading-9"><?php echo wp_kses_post($title); ?></h4>
					</div>
					<?php
					if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
						?>
						<div class="pt-5 pb-2 banner-countdown -mx-[5px]">
					...
					?>
					<div class="desc mt-4"><?php echo wp_kses_post($description); ?></div>
				</div>
				<div class="pt-3 md:text-left text-center">
					<?php
					foreach ( $buttons as $button ) {
						...
						<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
							...
							class="inline-block btn min-h-10 leading-[40px] pl-8 pr-[7px] rounded-[40px] rbb-button <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
						...
						<?php if ( $button_icon_status && 'before' === $button_icon_position ) { ?>
							<span class="button-icon md:mr-6 mr-3 w-8 h-8 leading-8 rounded-full text-center bg-black/20 inline-block align-middle">
								<?php
						...
						<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
							<span class="button-icon md:ml-6 ml-3 w-8 h-8 leading-8 rounded-full text-center bg-black/20 inline-block align-middle">
								<?php
								if ( ! empty($button_icon['value']) ) {
									Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
								} else {
		...
		<div class="col-span-12 md:col-span-6 lg:mt-0 mt-8">
