...
<div id="<?php echo esc_attr($id); ?>" class=" relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="mx-auto xl:pb-[170px] md:pb-20">
		<div class="banner-grid grid grid-cols-12 gap-4 bg-cover bg-no-repeat bg-center bg-[color:var(--rbb-general-primary-color)] bg-[url('../images/elementor/widgets/banner/bg-banner.png')]">
			<div class="content-left col-span-12 lg:col-span-6 relative py-24 xl:h-[700px] text-right">
				<div class="xl:w-[625px] w-full 2xl:px-0 px-[15px] text-left inline-block">
					<div class="text-white">
						<div class="title_block">
							<p class="sub_title xl:text-4xl text-xl mb-[3px]"><?php echo wp_kses_post($sub_title); ?></p>
							<div class="pt-9 pb-8">
							<?php
							if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
								?>
								<div class="rbb-countdown countdown-banner relative -mx-2.5 inline-flex justify-center text-center <?php echo esc_attr($countdown_position); ?>" data-countdown-date="<?php echo esc_attr($countdown); ?>">
									<div class="item-time w-1/4 px-2.5 relative days">
										<span class="data-number text-base text-xl text-black font-bold rounded-full bg-[#ffb137] block w-[58px] h-[58px] leading-[58px]">%D</span>
										<span class="name-time pt-2 text-[10px] text-white font-extrabold block uppercase"><?php echo esc_html__('Day%!H', 'botanica'); ?></span>
									</div>
									<div class="item-time w-1/4 px-2.5 relative">
										<span class="data-number text-base text-xl text-black font-bold rounded-full bg-[#ffb137] block w-[58px] h-[58px] leading-[58px]">%H</span>
										<span class="name-time pt-2 text-[10px] text-white font-extrabold block uppercase"><?php echo esc_html__('Hour%!H', 'botanica'); ?></span>
									</div>
									<div class="item-time w-1/4 px-2.5 relative">
										<span class="data-number text-base text-xl text-black font-bold rounded-full bg-[#ffb137] block w-[58px] h-[58px] leading-[58px]">%M</span>
										<span class="name-time pt-2 text-[10px] text-white font-extrabold block uppercase"><?php echo esc_html__('Min%!H', 'botanica'); ?></span>
									</div>
									<div class="item-time w-1/4 px-2.5 relative">
										<span class="data-number text-base text-xl text-black font-bold rounded-full bg-[#ffb137] block w-[58px] h-[58px] leading-[58px]">%S</span>
										<span class="name-time pt-2 text-[10px] text-white font-extrabold block uppercase"><?php echo esc_html__('Secs', 'botanica'); ?></span>
									</div>
								</div>
								<?php
							}
							?>
								...
								foreach ( $custom_html as $html ) {
									...
									echo "<div class='custom-html flex items-center justify-items-center xl:pb-10 lg:pb-5 pb-[30px] last:pb-0 " . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
								}
								?>
							</div>
							<h1 class="title"><?php echo wp_kses_post($title); ?></h1>
						</div>
						<div class="desc xl:mt-[34px] mt-6 max-w-[460px]"><?php echo wp_kses_post($description); ?></div>
					</div>
					<div class="pt-[43px]">
						<?php
						foreach ( $buttons as $button ) {
							...
							<?php if ( $button_icon_status && 'before' === $button_icon_position ) { ?>
								<span class="button-icon md:mr-6 mr-3 w-[42px] h-[42px] leading-[42px] !bg-transparent rounded-full text-center inline-block align-middle">
									<?php
							...
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon md:ml-6 ml-3 w-[42px] h-[42px] leading-[42px] !bg-transparent rounded-full text-center inline-block align-middle">
									<?php
									if ( ! empty($button_icon['value']) ) {
										Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
									} else {
							...
							if ( 'url' !== $button_type ) {
								?>
							<div id="rbb-modal-button-<?php echo esc_attr($button['_id']); ?>" class="rbb-modal <?php echo ( false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_CLICK_OUTSIDE_CLOSE) ) ? 'outside-modal' : ''; ?>" data-modal-animation="<?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_EFFECT)); ?>">
		...
		<div class="banner-left lg:order-1 -order-1 col-span-12 lg:col-span-6 lg:h-auto md:h-[400px] h-[350px]">
