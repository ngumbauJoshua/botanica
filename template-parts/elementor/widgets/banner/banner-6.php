...
<div id="<?php echo esc_attr($id); ?>" class=" relative overflow-hidden z-[1] <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="banner-video2 xl:h-[980px] md:h-[600px] h-[450px] flex justify-center items-center">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center">
				...
				<div class="content px-[15px] overflow-hidden">
					<div class="text-center">
						<div class="title_block">
							<h4 class="title lg:text-4xl text-3xl text-white md:leading-[55px] leading-9 wow fadeInUp"><?php echo wp_kses_post($title); ?></h4>
							<p class="sub_title lg:text-4xl text-3xl mb-0 pb-4 wow fadeInUp"><?php echo wp_kses_post($sub_title); ?></p>
						</div>
						<?php
						...
						?>
						<div class="desc lg:mt-1.5 text-base leading-[30px] text-white wow fadeInUp"><?php echo wp_kses_post($description); ?></div>
						<div class="lg:flex lg:justify-between">
							...
							foreach ( $custom_html as $html ) {
								...
								echo "<div class='custom-html text-center inline-block w-full" . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
							}
							?>
						</div>
					</div>
					<div class="pt-10 text-center">
						...
						foreach ( $buttons as $button ) {
							...
							<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
								...
								class="inline-block btn rbb-button rounded-full <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
								<?php
