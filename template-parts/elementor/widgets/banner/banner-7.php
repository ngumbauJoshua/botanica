...
<div id="<?php echo esc_attr($id); ?>" class=" relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="grid grid-cols-12 gap-4 items-center">
			<div class="content-left col-span-12 md:col-span-6 lg:col-span-3 order-1">
				<div class="text-left inline-block">
					<div class="title_block wow fadeInLeft" data-wow-delay="0.3s">
						<p class="sub_title text-base pb-[22px] mb-0"><?php echo wp_kses_post($sub_title); ?></p>
						<h4 class="title lg:text-[32px] text-3xl xl:leading-[48px] leading-9"><?php echo wp_kses_post($title); ?></h4>
					</div>
					<?php
					if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
						?>
						<div class="pt-5 pb-2 banner-countdown -mx-[5px] wow fadeInLeft" data-wow-delay="0.3s">
					...
					?>
					<div class="desc mt-[23px] text-white wow fadeInLeft" data-wow-delay="0.4s"><?php echo wp_kses_post($description); ?></div>
					<div class="pt-[43px]">
						<?php
						foreach ( $buttons as $button ) {
							...
							<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
								...
								class="inline-block btn h-14 leading-[52px] pl-8 pr-[7px] rounded-[56px] rbb-button <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
							<?php
							...
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon md:ml-6 ml-3 w-[42px] h-[42px] leading-[42px] rounded-full text-center inline-block align-middle">
									...
									if ( ! empty($button_icon['value']) ) {
										...
										?>
										<i class="rbb-icon-direction-711 text-[20px] leading-10 text-white" aria-hidden="true"></i>
									<?php } ?>
								</span>
							<?php } ?>
						</a>
		...
		<div class="banner-left col-span-12 lg:col-span-6 lg:order-2 -order-1">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center wow fadeInUp md:px-5">
				<?php
			...
			<div class="col-span-12 md:col-span-6 lg:col-span-3 order-3">
				<?php
				foreach ( $custom_html as $html ) {
					...
					echo "<div class='custom-html flex items-center justify-items-center pb-7 " . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
				}
				?>
			</div>
		</div>
