...
<div id="<?php echo esc_attr($id); ?>" class=" relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="grid grid-cols-12 gap-4 items-center">
			<div class="content-left col-span-12 lg:col-span-6 relative xl:pt-20 py-10 text-right">
				<div class="xl:w-[625px] w-full 2xl:pl-0 px-[15px] lg:text-left text-center inline-block">
					<div class="text-white">
						<div class="title_block">
							<p class="sub_title xl:text-2xl text-xl mb-[3px]"><?php echo wp_kses_post($sub_title); ?></p>
							<div class="pt-9 pb-8">
							<?php
							if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
								?>
								<div class="rbb-countdown countdown-banner relative border-dashed border border-[color:var(--rbb-general-secondary-color)] inline-flex justify-center rounded-[50px] pl-[26px] pr-5 <?php echo esc_attr($countdown_position); ?>" data-countdown-date="<?php echo esc_attr($countdown); ?>">
									<div class="item-time hidden"><span class="data-number">%D</span><span class="name-time"><?php echo esc_html__('Day%!H', 'botanica'); ?></span></div>
									<div class="item-time relative w-1/3 px-2 text-center whitespace-nowrap">
										<span class="data-number font-bold pr-1.5 leading-[50px] text-[color:var(--rbb-general-secondary-color)]">%H</span>
										<span class="name-time pt-2 text-[11px] text-white"><?php echo esc_html__('Hour%!H', 'botanica'); ?></span>
									</div>
									<div class="item-time relative w-1/3 px-2 text-center whitespace-nowrap">
										<span class="data-number font-bold pr-1.5 leading-[50px] text-[color:var(--rbb-general-secondary-color)]">%M</span>
										<span class="name-time pt-2 text-[11px] text-white"><?php echo esc_html__('Min%!H', 'botanica'); ?></span>
									</div>
									<div class="item-time relative w-1/3 px-2 text-center whitespace-nowrap">
										<span class="data-number font-bold pr-1.5 leading-[50px] text-[color:var(--rbb-general-secondary-color)]">%S</span>
										<span class="name-time pt-2 text-[11px] text-white"><?php echo esc_html__('Secs', 'botanica'); ?></span>
									</div>
								</div>
								<?php
							}
							foreach ( $custom_html as $html ) {
								...
								echo "<div class='custom-html flex items-center lg:justify-start justify-center xl:pb-10 lg:pb-5 pb-[30px] last:pb-0 " . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
							}
							?>
							</div>
							<h1 class="title xl:text-[60px] text-3xl xl:leading-[55px] leading-9"><?php echo wp_kses_post($title); ?></h1>
						</div>
						<div class="desc inline-block text-base xl:mt-[34px] mt-6 leading-[25px] max-w-[460px]"><?php echo wp_kses_post($description); ?></div>
					</div>
					<div class="pt-[43px]">
						<?php
						foreach ( $buttons as $button ) {
							...
							<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
								...
								class="inline-block btn h-14 leading-[55px] pl-8 pr-[7px] rounded-[56px] rbb-button <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
								<?php
							...
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon md:ml-6 ml-3 w-[42px] h-[42px] leading-[42px] rounded-full text-center inline-block align-middle">
									<?php
									if ( ! empty($button_icon['value']) ) {
										Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
									} else {
										?>
										<i class="rbb-icon-direction-711 text-[20px] leading-10" aria-hidden="true"></i>
									<?php } ?>
								</span>
							<?php } ?>
							</a>
		...
		<div class="banner-left col-span-12 lg:col-span-6 lg:mt-0 mt-8 xl:pb-0 pb-20">
