...
<div id="<?php echo esc_attr($id); ?>" class=" relative z-[1] <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="banner-product relative md:pb-[180px]">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center">
				...
				<div class="content absolute w-full px-[15px] z-10 top-[50px] left-1/2 -translate-x-1/2">
					<div class="text-center">
						<div class="title_block">
							<h4 class="title text-2xl text-white leading-7 pb-3"><?php echo wp_kses_post($title); ?></h4>
							<p class="sub_title text-2xl"><?php echo wp_kses_post($sub_title); ?></p>
						</div>
						<?php
						...
						?>
						<div class="desc lg:mt-4 text-base leading-6 text-white"><?php echo wp_kses_post($description); ?></div>
						<div class="md:flex mt-2 md:justify-between">
							<?php
					...
					<div class="text-center">
						...
						foreach ( $buttons as $button ) {
							...
							<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
								...
								class="inline-block btn <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
								<?php
							...
							<span class="button-text inline-block align-middle">
								<?php echo wp_kses_post($button_name); ?>
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon text-center inline-block align-middle">
									<?php
									if ( ! empty($button_icon['value']) ) {
										Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
									} else {
										?>
										<i class="rbb-icon-direction-711 pl-1.5" aria-hidden="true"></i>
									<?php } ?>
								</span>
							<?php } ?>
							</span>
