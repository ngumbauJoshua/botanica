...
<div id="<?php echo esc_attr($id); ?>" class="banner-image relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="grid relative">
			<div class="w-full absolute -translate-x-1/2 left-1/2 z-10 text-center bottom-0 px-5">
				<div class="title_block">
					<p class="sub_title text-3xl text-white mb-2"><?php echo wp_kses_post($sub_title); ?></p>
					<h4 class="title text-xl text-white font-bold"><?php echo wp_kses_post($title); ?></h4>
				</div>
				<?php
				...
				<div class="flex items-center mt-[34px]">
					...
					foreach ( $buttons as $button ) {
						...
						<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
							<span class="button-icon ml-[16px] ms:ml-3 w-[35px] h-[35px] leading-[35px] rounded-full text-center bg-black/20 inline-block align-middle text-[#fff]">
								<?php
								if ( ! empty($button_icon['value']) ) {
									Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
								} else {
		...
		<div class="col-span-12 lg:col-span-12">
			<div class="relative text-center">
				...
				foreach ( $images as $image ) {
					...
					?>
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img md:block inline-block">
							<?php
							if ( $image_link_attr_string ) {
								?>
							<a <?php echo wp_kses($image_link_attr_string, 'rbb-kses'); ?> >
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="scale-100 hover:scale-105 duration-300 <?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
