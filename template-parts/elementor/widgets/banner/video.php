...
<div id="<?php echo esc_attr($id); ?>" class=" relative z-[1] <?php echo esc_attr($layout); ?>">
	<div class="mx-auto">
		<div class="banner-video relative">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> text-center">
				<?php
				$img_count = 0;
				foreach ( $images as $image ) {
					...
					?>
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img absolute w-full h-full top-0 left-0 -z-[1]">
							<?php
				...
				<div class="content w-full mx-auto max-w-[800px] px-[15px] xl:py-[208px] md:py-[140px] py-[120px]">
					<div class="text-center">
						<div class="title_block">
							<h2 class="title xl:text-4xl text-2xl text-white leading-[55px] mb-5"><?php echo wp_kses_post($title); ?></h2>
							<h3 class=" sub_title xl:text-4xl text-xl pb-4"><?php echo wp_kses_post($sub_title); ?></h3>
						</div>
						<?php
						...
						?>
						<div class="desc lg:mt-4 text-base text-white"><?php echo wp_kses_post($description); ?></div>
						<div class="md:flex xl:mt-10 mt-2 md:justify-between">
							<?php
					...
					<div class="pt-10 pb-5 text-center">
						...
						foreach ( $buttons as $button ) {
							...
							<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
								...
								class="inline-block btn rbb-button h-16 rounded-full <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
								<?php
							...
							<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
								<span class="button-icon w-16 h-16 leading-[64px] rounded-full text-center bg-black/20 inline-block align-middle">
									<?php
									if ( ! empty($button_icon['value']) ) {
										Icons_Manager::render_icon($button_icon, [ 'aria-hidden' => 'true' ]);
									} else {
										?>
										<i class="rbb-icon-direction-21 text-white" aria-hidden="true"></i>
									<?php } ?>
								</span>
							<?php } ?>
						</a>
							...
							<div id="rbb-modal-button-<?php echo esc_attr($button['_id']); ?>" class="rbb-modal <?php echo ( false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_CLICK_OUTSIDE_CLOSE) ) ? 'outside-modal' : ''; ?>" data-modal-animation="<?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_EFFECT)); ?>">
								<div class="rbb-modal-dialog overflow-hidden">
