...
if ( count($posts['posts']) ) {
	?>
	<div id="<?php echo esc_attr($id); ?>" class="rbb_posts bg-cover overflow-hidden <?php echo esc_attr($layout); ?> pb-[67px]">
		<div class="container mx-auto mt-2">
			<div class="block_slide lg:-ml-7">
				<div class="block_slider__content lg:pl-7 overflow-hidden">
					<div class="uppercase font-extrabold lg:text-4xl text-3xl mb-[37px] flex items-center md:justify-between justify-center text-white">
						<h2 class="title"><?php echo wp_kses_post($posts['title']); ?></h2>
						...
						if ( isset($posts['type']) && 'category' === $posts['type'] ) {
							...
							if ( 1 < $cat_count || ( $show_filter && 1 === $cat_count ) ) {
								?>
								<span class="title-of hidden sm:block text-sm uppercase mx-8"><?php echo esc_html__('Of', 'botanica'); ?></span>
