...
<div class="item px-[15px] basis-0 grow">
	<div class="blog_item duration-300 hover:md:translate-y-[-15px] hover:shadow-[5px_5px_05px_rgba(0,0,0,0.15)] group overflow-hidden bg-white">
			...
			<a href="<?php echo esc_url(get_permalink($_post)); ?>" class="w-full">
				...
				if ( has_post_thumbnail($_post) ) {
					...
					<img class="object-cover duration-300 scale-100 group-hover:scale-105 xl:min-h-[300px]"
						...
						alt="<?php echo esc_html__('Default post thumbnail', 'botanica'); ?>">
					<?php
				}
				?>
			</a>
		...
		<div class="blog_info mb-[23px]">
			...
			<?php if ( $show_read_more ) { ?>
				<a href="<?php echo esc_url(get_permalink($_post)); ?>" class="blog_readmore text-[bebebe] font-semibold relative mt-6 mb-4 pt-3 pr-[15px] pb-[13px]"><span class="mr-1.5 relative"><?php echo esc_html__('Read more', 'botanica'); ?></span><i class="rbb-icon-direction-711 align-middle text-lg leading-[17px] relative"></i></a>
			<?php } ?>
		</div>
	</div>
</div>
