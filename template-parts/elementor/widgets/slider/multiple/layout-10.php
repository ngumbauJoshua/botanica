...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<div class="mx-auto wow fadeInUp" data-wow-delay="0.1s">
			<div class="content-grid items-center relative">
				<div class="block_title text-center">
					<?php if ( $widget_title || $widget_sub_title ) { ?>
						<div class="title_block mb-10">
							<?php
							if ( $widget_sub_title ) {
								?>
								<p class="sub-title text-white text-2xl pb-3 mb-0"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
							<?php
							if ( $widget_title ) {
								?>
								<h4 class="main-title text-xl"><?php echo esc_html($widget_title); ?></h4> <?php } ?>
						</div>
					<?php } ?>
				</div>
				<div class="block_slide">
					<div class="rbb-slick-carousel slick-carousel slick-carousel-center load-item-<?php echo esc_attr($sliders_to_show_default); ?>" data-slick='{
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item mb-10 group duration-300">
								<div class="duration-300 border-b border-b-[#e5e5e5] border-r border-r-[#e5e5e5] hover:bg-[color:var(--rbb-general-primary-color)]">
									<div class="block__content duration-300 relative text-center px-3 md:pt-20 pt-10 pb-8  hover:bg-[color:var(--rbb-general-primary-color)]">
										<div class="hover-content duration-300">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<div class="md:min-h-[220px] min-h-[150px] flex justify-center items-center mb-7">
											<img class="!inline-block md:!max-w-[210px] md:!max-h-[210px] max-h-[150px]" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
										</div>
										<?php
									}
									?>
									<p class="text-[color:var(--rbb-general-primary-color)] group-hover:text-[color:var(--rbb-general-secondary-color)] font-bold duration-300 mb-2.5"><?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></p>
									<h4 class="text-base pb-2.5">
										<?php
										if ( 'url' === $button_type || $content_link_attr_string ) {
											?>
											<a class="group-hover:text-white duration-300" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
									...
									if ( $button ) {
										...
										<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-multiple-button\', event)"' : ''; ?>
												...
												class="rbb-slick-button w-[62px] h-[62px] absolute bottom-0 opacity-0 left-1/2 translate-x-[-50%] translate-y-[50%] btn_view-all duration-300 rounded-full inline-flex justify-center items-center">
												<span class="rounded-full w-[50px] h-[50px] text-lg text-white inline-flex justify-center items-center bg-[color:var(--rbb-general-secondary-color)] hover:bg-[color:var(--rbb-general-primary-color)]">
													<?php
													if ( $button_icon ) {
										...
										if ( 'url' !== $button_type ) {
											?>
											<div id="rbb-modal-multiple-button" class="rbb-modal <?php echo ( false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_CLICK_OUTSIDE_CLOSE) ) ? 'outside-modal' : ''; ?>" data-modal-animation="<?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_EFFECT)); ?>">
