...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="max-w-[1330px] lg:px-5 px-[15px] mx-auto overflow-hidden">
					...
					<div class="block_slide">
						...
						foreach ( $sliders as $slider ) {
							...
							<div class="item md:py-[60px] sm:py-10">
								<div class="block__content relative duration-300 bg-white text-center rounded-[26px] px-3 lg:px-4 pt-4 pb-14 shadow-[11px_10px_10px_0px_rgba(0,0,0,0.1)]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<img class="mb-5 !rounded-[26px] !inline-block" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
										<?php
									}
									?>
									...
									if ( $button ) {
										...
										<span class="rounded-full w-[50px] h-[50px] text-lg text-white inline-flex justify-center items-center bg-[color:var(--rbb-general-secondary-color)]">
											<?php
											if ( $button_icon ) {
