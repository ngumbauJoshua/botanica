		...
		<div class="container mx-auto px-[15px] overflow-hidden">
			<div class="block_title text-center flex justify-center items-center">
				<?php if ( 'true' === $show_arrows ) { ?>
					<div class="slick-arrow-custom md:block hidden text-lg text-white">
				...
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block font-bold mb-[33px]">
						<?php
						if ( $widget_sub_title ) {
							?>
							<p class="sub-title text-lg pb-2 mb-0 wow fadeInUp text-[color:var(--rbb-general-primary-color)]"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
							<?php
							if ( $widget_title ) {
								?>
								<h4 class="main-title md:text-[2.5rem] text-3xl wow fadeInUp"><?php echo esc_html($widget_title); ?></h4> <?php } ?>
							</div>
						<?php } ?>
						<?php if ( 'true' === $show_arrows ) { ?>
							<div class="slick-arrow-custom md:block hidden text-lg text-white">
					...
					<div class="block_slide wow fadeInUp">
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item">
								<div class="block__content relative duration-300  text-center px-3 mt-3">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<div class="hex-image text-center relative p-5 duration-300">
									...
									<?php } ?>
									<h4 class="text-xl font-bold pb-4 pt-[45px]">
										<?php
										if ( 'url' === $button_type || $content_link_attr_string ) {
											?>
											<a class="text-[color:var(--rbb-general-primary-color)]" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
									...
									<p class="text-center leading-6 mb-0"><?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></p>
