...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<div class="container mx-auto px-[15px] overflow-hidden">
			<div class="block_title text-center flex justify-center items-center mb-3">
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block">
						<?php
						if ( $widget_sub_title ) {
							?>
							<p class="sub-title font-bold text-lg pb-2 mb-0"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
							<?php
							if ( $widget_title ) {
								?>
								...
								<div class="item md:py-[60px] sm:py-10">
									<div class="block__content relative duration-300 text-center rounded-[26px] px-3 lg:px-4 pt-4 pb-[66px]">
										<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
											<div class="xl:min-h-[341px] min-h-[250px] mb-5">
										...
										<h4 class="text-base font-bold pb-2.5">
											...
											if ( 'url' === $button_type || $content_link_attr_string ) {
												?>
												<a class="duration-300 text-white hover:text-[color:var(--rbb-general-link-hover-color)]" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
										...
										<p class="text-center max-w-[260px] mb-0 inline-block text-white"><?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></p>
				...
				<div class="swiper-scrollbar !w-full !h-[1px] !left-0 !bottom-[3px] !relative !bg-[color:var(--rbb-general-primary-color)]"></div>
		</div>
	</div>
<?php } ?>
