...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="container mx-auto px-[15px] overflow-hidden">
			<div class="block_title text-center flex justify-center items-center">
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block">
							...
							if ( $widget_title ) {
								...
								if ( $widget_sub_title ) {
									?>
									<p class="sub-title pt-2"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
							</div>
						<?php } ?>
					</div>
					<div class="block_slide swiper-right overflow-hidden">
						<div class="rbb-slider-swiper swiper-container" data-swiper='{
							"scrollbar" : {
						...
						foreach ( $sliders as $slider ) {
							...
							<div class="swiper-slide">
								<div class="item md:py-[60px] sm:py-5">
									<div class="block__content relative duration-300 text-center rounded-[26px] px-3 lg:px-4 pt-4 pb-[66px]">
										<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
											<div class="mb-4 max-w-[150px] inline-block">
										...
										<h4 class="text-base pb-2.5">
											...
											if ( 'url' === $button_type || $content_link_attr_string ) {
												?>
												<a class="duration-300 hover:text-[color:var(--rbb-general-link-hover-color)]" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
										...
										<p class="text-center max-w-[260px] mb-0 inline-block"><?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></p>
									<?php } ?>
										<?php
										if ( $button ) {
											...
											class="rbb-slick-button w-[62px] h-[62px] absolute bottom-[30px] opacity-0 left-1/2 translate-x-[-50%] translate-y-[50%] btn_view-all rounded-full inline-flex justify-center items-center">
				...
				<div class="swiper-scrollbar !w-full !h-[1px] !left-0 !bottom-[3px] !relative !bg-[color:var(--rbb-general-secondary-color)]"></div>
		</div>
	</div>
<?php } ?>
