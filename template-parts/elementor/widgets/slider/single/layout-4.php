...
if ( $sliders ) {
	?>
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>" data-speed="<?php echo esc_attr($autoplay_speed); ?>">
		<div class="flex rbb-slick-slider rbb-slick-carousel slick-carousel slick-carousel-center load-item" data-slick='{
			...
			"fade": true,
			"appendDots": "#<?php echo esc_attr($id); ?> .slick-slider-dots",
						...
						<div class="md:absolute md:py-0 pt-9 pb-6 top-0 w-full h-full">
							<div class="container <?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> m-auto relative h-full">
								...
								if ( $slider_title || $slider_sub_title || $description || $button_1 || $button_2 ) {
									...
									?>
									<div class="absolute info-wap md:w-[45%] w-[60%] w-1/2 z-50 <?php echo esc_attr($horizontal_align_class); ?> <?php echo esc_attr($vertical_align_class); ?>">
										<div class="info-inner pl-[15px]">
											<?php
											...
											if ( $slider_title ) {
												...
												<h2 data-visibility-start="<?php echo esc_attr($title_visibility); ?>"
													...
													class="title sm:text-base md:text-xl lg:text-4xl mb-4 <?php echo esc_attr($title_visibility); ?>"
												>
											...
											if ( $description ) {
												...
												<div data-visibility-start="<?php echo esc_attr($description_visibility); ?>"
													...
													class="description font-medium max-w-[470px]  md:mb-4 lg:whitespace-normal lg:mb-6 <?php echo esc_attr($description_visibility); ?>"
												>
											...
											?>
											<div class="button-group flex items-center text-left <?php echo ( 'center' !== $horizontal_align_center ) ? '' : 'justify-center'; ?>">
												<?php
												if ( $button_1 ) {
		...
		<div id="slideshow-dot" class="flex justify-center absolute md:bottom-[90px] bottom-12 left-0 w-full z-[1]">
			<?php
			$i = 0;
			foreach ( $sliders as $slider ) {
				?>
				<div class="relative h-7 w-7 cursor-pointer mx-[2px] <?php echo ( 0 === $i ) ? 'active' : ''; ?>" data-index="<?php echo esc_attr($i); ?>"><svg><circle cx="14" cy="14" r="10" fill="none" stroke-width="8"></circle></svg></div>
				<?php $i++; ?>
			<?php } ?>
		</div>
		<?php } ?>
