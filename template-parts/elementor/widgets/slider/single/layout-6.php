...
if ( $sliders ) {
	?>
	<div class="rbb-elementor-slider pb-16 <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		<?php if ( $widget_title || $widget_sub_title ) { ?>
		<div class="title_block text-black text-center font-bold mb-[54px] pb-5 relative">
			<?php
			if ( $widget_title ) {
				?>
				<h2 class="main-title mb-1.5"><?php echo esc_html($widget_title); ?></h2> <?php } ?>
			<?php
			if ( $widget_sub_title ) {
				?>
				<p class="sub-title text-base text-[color:var(--rbb-general-primary-color)]"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
		</div>
		<?php } ?>
		<div class="flex rbb-slick-carousel slick-carousel slick-carousel-center load-item" data-slick='{
			...
			foreach ( $sliders as $slider ) {
				...
				?>
				<div class="w-full mt-[30px] item relative elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?>">
					<?php
					...
					if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
						?>
					<div class="max-w-[270px] absolute right-1/2 translate-x-1/2 -top-[30px] z-10">
					...
					<div class="item-content relative overflow-hidden md:rounded-[500px] rounded-[20px] mx-[15px]">
						<div class="md:absolute md:py-0 pt-14 pb-6 top-0 w-full h-full">
							<div class="container <?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> m-auto relative h-full">
								...
								if ( $slider_title || $slider_sub_title || $description || $button_1 || $button_2 ) {
									...
									?>
									<div class="md:absolute info-wap xl:pl-[100px] md:pl-[70px] xl:w-[42%] md:w-1/2 z-50 <?php echo esc_attr($horizontal_align_class); ?> <?php echo esc_attr($vertical_align_class); ?>">
										<div class="info-inner md:pl-[15px]">
											...
											if ( $slider_sub_title ) {
												...
												<h3 data-visibility-start="<?php echo esc_attr($subtitle_visibility); ?>"
													...
													class="sub-title text-xs mb-5 <?php echo esc_attr($subtitle_visibility); ?>"
												>
											...
											if ( $slider_title ) {
												...
												<h2 data-visibility-start="<?php echo esc_attr($title_visibility); ?>"
													...
													class="title text-2xl lg:text-3xl mb-2 <?php echo esc_attr($title_visibility); ?>"
												>
											...
											if ( $description ) {
												...
												<div data-visibility-start="<?php echo esc_attr($description_visibility); ?>"
													...
													class="description md:truncate lg:whitespace-normal mb-[30px] <?php echo esc_attr($description_visibility); ?>"
												>
											...
											?>
											<div class="button-group md:flex items-center md:text-left text-center <?php echo ( 'center' !== $horizontal_align_center ) ? '' : 'justify-center'; ?>">
												<?php
												if ( $button_1 ) {
													...
													<a <?php echo ( 'url' !== $button_1_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-01-' . esc_attr($slider['_id']) . '\', event)"' : ''; ?>
														...
														class="rbb-slick-button duration-300 button-1 pl-6 pr-2 py-2 rounded-full inline-block <?php echo esc_attr($button_1_visibility); ?>"
														<?php
													...
													>
														...
														<span class="button-icon inline-block align-middle xl:ml-4 ml-2 xl:w-10 xl:h-10 xl:leading-10 w-8 h-8 leading-8 rounded-full">
														<?php
														if ( $button_1_icon ) {
															Icons_Manager::render_icon($button_1_icon, [ 'aria-hidden' => 'true' ]);
															?>
														<?php } else { ?>
															<i class="rbb-icon-direction-711 text-xl xl:leading-10 leading-8" aria-hidden="true"></i>
														<?php } ?>
														</span>
													</a>
													<?php
													if ( 'url' !== $button_1_type ) {
														?>
														<div id="rbb-modal-button-01-<?php echo esc_attr($slider['_id']); ?>" class="rbb-modal <?php echo ( false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_CLICK_OUTSIDE_CLOSE) ) ? 'outside-modal' : ''; ?>" data-modal-animation="<?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_EFFECT)); ?>">
									...
									if ( $slider[ $slick->get_name_setting('image_' . $i) ] && $slider[ $slick->get_name_setting('image_' . $i) ]['url'] ) {
										...
										<img
											...
											<?php echo ! empty($animation_delay) ? 'data-animation-delay="' . esc_attr($animation_delay) . '"' : ''; ?>
												class="xl:!max-w-[503px] xl:max-h-full <?php echo esc_attr(( 'absolute' === $position ) ? '!absolute' : '!relative'); ?>
												<?php
												if ( $parallax && 1 !== $i ) {
													echo 'layer'; }
												?>
