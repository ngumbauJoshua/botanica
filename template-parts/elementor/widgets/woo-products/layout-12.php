	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products overflow-hidden <?php echo esc_attr($layout); ?>">
		...
		<div class="max-w-[1340px] xl:px-[15px] pb-3 mx-auto overflow-hidden">
			...
			<div class="rbb-product-list grid gap-0 xl:grid-cols-3 grid-cols-2">
				...
				$start_index    = 0;
				$end_index      = $count - 1;
				$max_loop_count = min($count, $start_index + 3);
				?>
				<div class="col-span-1 block_left xl:order-[0]">
					<?php
					for ( $i = $start_index; $i < $max_loop_count; $i++ ) {
						View::instance()->load(
				...
				if ( $count >= 4 ) {
					?>
					<div class="block_center xl:col-span-1 col-span-2 xl:order-1 -order-1">
						<?php
						View::instance()->load(
				...
				?>
				<div class="col-span-1 block_right xl:order-3">
					<?php
					for ( $i = max($start_index, $count - 3); $i <= $end_index; $i++ ) {
						View::instance()->load(
							'elementor/widgets/woo-products/fragments/item-3',
							[
								'product' => $data['products'][ $i ],
