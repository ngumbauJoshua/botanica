...
if ( count($data['products']) ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="container mx-auto relative mb-[50px]">
			<div class="sm:flex title_left items-center justify-between items-center mb-3">
				<?php if ( $show_title ) { ?>
				<div class="md:text-left text-center md:flex-1 flex-none md:mb-0 mb-5">
					<h2 class="title"><?php echo wp_kses_post($data['title']); ?></h2>
					<span class="sub-title block"><?php echo wp_kses_post($subtitle); ?></span>
				</div>
			<?php } ?>
				<div class="xl:flex-1 tab shrink-0">
					<div class="flex-wrap md:justify-end justify-center items-center md:flex text-sm">
						<div class="lg:pr-7">
							<?php
							if ( 'category' === $data['type'] ) {
								...
								if ( '1' < $cat_count || ( $show_filter && '1' === $cat_count ) ) {
									...
									data-ajax='{
										...
										"show_wishlist" : <?php echo esc_attr(( $show_wishlist ) ? 'true' : 'true'); ?>,
										"show_rating" : <?php echo esc_attr(( $show_rating ) ? 'true' : 'false'); ?>,
										"show_quickview" : <?php echo esc_attr(( $show_quickview ) ? 'true' : 'false'); ?>,
										"show_compare" : <?php echo esc_attr(( $show_compare ) ? 'true' : 'false'); ?>,
										"show_add_to_cart" : <?php echo esc_attr(( $show_add_to_cart ) ? 'true' : 'false'); ?>,
										"show_countdown" : <?php echo esc_attr(( $show_countdown ) ? 'true' : 'false'); ?>
									}'
									>
					...
					<?php if ( $show_arrows ) { ?>
						<div class="arrow-custom block_btn md:flex hidden">
				...
				<div class="block_slide swiper-right overflow-hidden pl-4 -ml-4">
					<div class="rbb-swiper swiper-container pb-[68px]"
					data-swiper='{
