<?php
/**
 * The default header.
 *
 * @package Rising_Bamboo
 */

use RisingBambooTheme\App\App;
use RisingBambooTheme\Helper\Helper;
use RisingBambooTheme\Helper\Setting;
use RisingBambooTheme\Helper\Tag;
use RisingBambooTheme\App\Menu\Menu;
use RisingBambooTheme\Woocommerce\Woocommerce as RisingBambooWoo;
$modal_effect = Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_MODAL_EFFECT);
?>
<header id="rbb-default-header" class="rbb-default-header header-3 lg:absolute w-full z-20">
	<div class="rbb-header-sticky relative md:h-[100px] h-[70px] md:block hidden">
		<div class="md:container mx-auto h-full md:px-[15px] px-5">
			<div class="header-inner relative h-full lg:px-[30px] lg:rounded-b-[35px] bg-[color:var(--rbb-header-background-color)]">
				<div class="flex flex-nowrap items-center justify-between h-full">
					<?php
					if ( Helper::show_logo() ) {
						?>
						<div id="rbb-branding" class="rbb-branding rbb-header-left flex-1 flex flex-none items-center py-4 xl:mr-32 md:mr-2">
							<span class="toggle-megamenu w-[18px] h-[14px] relative mr-4 cursor-pointer lg:hidden">
								<i class="icon-directional"></i>
							</span>
							<div id="_desktop_logo" class="flex items-center">
								<a href="<?php echo site_url(); ?>">
									<?php
									$logo        = Tag::get_logo_uri();
									$logo_sticky = Tag::get_logo_uri('sticky');
									if ( $logo === $logo_sticky ) {
										?>
										<img class="logo" src="<?php echo esc_url($logo); ?>" alt="<?php echo esc_html__('logo', 'botanica'); ?>">
										<?php
									} else {
										?>
										<img class="sticky-logo" src="<?php echo esc_url($logo_sticky); ?>" alt="<?php echo esc_html__('Sticky Logo', 'botanica'); ?>">
										<img class="logo" src="<?php echo esc_url($logo); ?>" alt="<?php echo esc_html__('logo', 'botanica'); ?>">
									<?php } ?>
								</a>
							</div>
						</div>
					<?php } ?>
					<div id="desktop_menu" class="md:flex-1 rbb-header-center lg:block hidden">
						<div class="search_desktop animate-[500ms] lg:hidden"></div>
						<div id="rbb-site-navigation" class="rbb-main-navigation screen">
							<nav id="menu-main" class="menu primary-menu">
								<?php echo Menu::primary_menu(); // phpcs:ignore ?>
							</nav>
							<div class="lg:hidden mt-8 items-center mobile_bottom">
								<div class="flex mb-1.5 animate-[1500ms]">
									<span class="pr-1 font-bold"><?php echo esc_html__('Email:', 'botanica'); ?> </span>
									<?php echo do_shortcode('[rbb_contact type="phone"]'); ?>
								</div>
								<div class="flex mb-[25px] animate-[1600ms]">
									<span class="pr-1 font-bold"><?php echo esc_html__('Call Us:', 'botanica'); ?></span>
									<?php echo do_shortcode('[rbb_contact type="email"]'); ?>
								</div>
								<div class="animate-[1700ms] social_content"></div>
							</div>
						</div>
					</div>
					<?php
					if ( Helper::show_login_form() || Helper::show_search_product_form() || Helper::show_mini_cart() || Helper::show_wishlist() ) {
						?>
						<div id="rbb-header-right" class="rbb-header-right flex-none h-full flex min-w-[80px] justify-end">
							<?php if ( Helper::show_search_product_form() ) { ?>
								<div class="rbb-product-search relative flex items-center justify-center cursor-pointer" onclick="RbbThemeSearch.openSearchForm(event)">
									<div class="rbb-product-search-icon-wrap justify-center flex items-center <?php echo ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_ICON_BORDER) > 0 ) ? 'h-[50px] w-[50px]' : ''; ?>">
										<span class="rbb-product-search-icon <?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_ICON)); ?>"></span>
									</div>
									<div class="rbb-product-search-content mr-5">
										<span class="ml-[10px] text-white text-xs font-medium uppercase"><?php echo esc_html__('Search', 'botanica'); ?></span>
										<?php if ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
											<?php Tag::search(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY)); // phpcs:ignore ?>
										<?php } else { ?>
											<div id="rbb-search-content" class="absolute left-auto right-[50px] top-1/2 -translate-y-1/2 invisible opacity-0 ">
												<div id="search-mobile" class="rbb-search-top relative">
													<div onclick="RbbThemeSearch.closeSearchForm(event)" class="close-search absolute top-2 -right-10 lg:block hidden">
														<i class="rbb-icon-close-1 text-lg"></i>
													</div>
													<div id="_desktop_search">
														<?php Tag::search(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY)); // phpcs:ignore ?>
													</div>
												</div>
											</div>
										<?php } ?>
									</div>
								</div>
							<?php } ?>
							<?php if ( Helper::show_login_form() ) { ?>
								<div class="rbb-account relative flex items-center justify-center pl-5 mr-[30px]
								<?php echo ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) ? 'popup-account' : 'toggle-login'; ?> "
								<?php
								if ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) {
									?>
									onclick="RisingBambooModal.modal('.account-form-popup', event)" <?php } ?>
									>
									<div class="rbb-account-icon-wrap relative justify-center items-center flex duration-300 cursor-pointer <?php echo ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_ICON_BORDER) > 0 ) ? 'h-[50px] w-[50px]' : ''; ?>">
										<span class="rbb-account-icon <?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_ICON)); ?>"></span>
									</div>
									<div class="rbb-account-content-wrap">
										<?php
										$content = '';
										if ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) {
											$welcome = __('Account', 'botanica');
											if ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) {
												add_action(
													'wp_footer',
													function () use ( $modal_effect ) {
                                                   // phpcs:ignore
                                                       echo '<div class="account-form-popup rbb-modal invisible fixed inset-0 z-50 " data-modal-animation="' . esc_attr($modal_effect) . '">' . Tag::login_form() . '</div>';
													}
												);
											}
										} else {
											$content = Menu::account_menu();
											if ( ! empty($content) ) {
												?>
											<div class="rbb-account-content duration-300 mt-14 absolute z-10 visibility invisible rounded-lg overflow-hidden opacity-0 top-full right-0 shadow-[6px_5px_11px_rgba(0,0,0,0.1)]">
												<div class="relative bg-white min-w-[220px] px-8 pt-3 pb-5">
													<?php echo wp_kses($content, 'rbb-kses'); ?>
												</div>
											</div>
												<?php
											} 
										}
										?>
									</div>
								</div>
							<?php } ?>

							<?php if ( Helper::show_wishlist() ) { ?>
								<div class="rbb-wishlist md:flex hidden items-center justify-center mr-2">
									<div class="relative">
										<a class="wishlist-icon-link duration-300 h-[50px] w-[50px] flex items-center justify-center text-center" aria-label="wishlist" href="<?php echo esc_url(WPcleverWoosw::get_url()); ?>">
											<span class="wishlist-icon <?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_WOOCOMMERCE_WISHLIST_ICON)); ?>"></span>
										</a>
										<span class="wishlist-count text-center rounded-full"><?php echo WPcleverWoosw::get_count(); // phpcs:ignore ?></span>
									</div>
								</div>
							<?php } ?>

							<?php
							if ( Helper::show_mini_cart() ) {
								RisingBambooWoo::instance()->mini_cart();
							}
							?>
						</div>
					<?php } ?>
				</div>
			</div>
		</div>
	</div>
	<div class="header-mobile relative bg-white px-[15px] md:hidden">
		<div class="flex items-center justify-between max-h-[60px]">
			<div class="menu-mobile flex items-center min-w-[60px] h-[60px]">
				<span class="toggle-megamenu w-[18px] h-[14px] relative mr-5 cursor-pointer">
					<i class="icon-directional"></i>
				</span>
				<div class="w-[22px] text-center">
					<span class="search-mobile text-center text-black w-[18px] h-[18px] flex justify-center items-center cursor-pointer text-[22px]">
						<i class="rbb-icon-search-10"></i>
					</span>
					<div id="_mobile_search" class="product-search-mobile absolute z-10 inset-x-0 top-full w-full opacity-0 invisible">
					</div>
				</div>
			</div>
			<div id="_mobile_logo" class="px-5 flex items-center justify-center"></div>
			<div class="header-mobile-right min-w-[60px] flex items-center justify-end">
				<div id="_mobile_cart" class="rbb-mini-cart"></div>
			</div>
		</div>
	</div>
</header><!-- #masthead -->
