...
?>
<header id="rbb-default-header" class="rbb-default-header header-6 md:absolute w-full z-20">
	<div class="rbb-header-sticky relative lg:h-[118px] md:h-20 h-[70px] md:block hidden">
		<div class="header-inner relative mx-auto h-full px-5 xl:px-7 2xl:px-10 bg-[color:var(--rbb-header-background-color)]">
			<div class="flex flex-nowrap items-center flex h-full grid grid-cols-8 gap-4">
				<div id="desktop_menu" class="rbb-header-center lg:col-span-3 col-span-3 md:-mr-9 lg:block hidden">
				...
				if ( Helper::show_logo() ) {
					?>
					<div id="rbb-branding" class="rbb-branding rbb-header-left md:flex md:items-center lg:justify-center lg:text-center lg:col-span-2 col-span-4">
				...
				if ( Helper::show_login_form() || Helper::show_search_product_form() || Helper::show_mini_cart() || Helper::show_wishlist() ) {
					?>
					<div id="rbb-header-right" class="rbb-header-right flex-none h-full ml-3 flex ml-auto mr-0 lg:col-span-3 col-span-4 <?php echo Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ? 'border-cart' : ''; ?>">
						...
						<?php if ( Helper::show_search_product_form() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
							<div class="rbb-product-search relative flex items-center justify-center cursor-pointer" onclick="RbbThemeSearch.openSearchForm(event)">
								<div class="rbb-product-search-content2 results-full">
									<div id="search-mobile" class="rbb-product-search-icon-wrap bg-white h-[50px] w-[50px] items-center justify-center flex mr-2.5">
