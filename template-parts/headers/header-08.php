...
<header id="rbb-default-header" class="rbb-default-header header-8 w-full relative z-20">
	<div class="rbb-header-sticky relative md:block hidden">
		<div class="header-inner md:container mx-auto md:h-[110px] h-[70px]">
			<div class="flex flex-nowrap items-center flex h-full grid grid-cols-8 md:gap-4">
				<div class="rbb-header-center flex items-center md:col-span-3 col-span-2">
					...
					<?php
					$phone = do_shortcode('[rbb_contact type="phone"]');
					$email = do_shortcode('[rbb_contact icon="rbb-icon-email-11" type="email"]');
					if ( $phone || $email ) { 
						?>
						<div class="policy md:flex hidden items-center">
							<?php if ( $phone || $email ) { ?>
				...
				if ( Helper::show_login_form() || Helper::show_mini_cart() || Helper::show_wishlist() ) {
					...
					<div id="rbb-header-right" class="rbb-header-right flex-none h-full max-w-[390px] ml-3 flex ml-auto mr-0 md:col-span-3 col-span-2">
						...
						?>
						<div class="rbb-category flex items-center justify-center all-categories hidden">
							<span class="category-mobile text-black lg:!hidden !block h-[50px] flex items-center justify-center text-center w-[22px] py-3 ml-5 cursor-pointer text-black text-[22px]">
		...
		<div class="rbb-header-center relative bg-[color:var(--rbb-header-background-color)]">
			<div class="container mx-auto min-h-[70px] lg:flex hidden items-center justify-between">
				<div class="xl:min-w-[300px] all-categories hidden">
					<div class="category-mobile duration-300 cursor-pointer !flex items-center text-white min-h-[70px] md:max-w-[200px] min-w-[200px] border-r border-[#1f303d]">
						<i class="rbb-icon-menu-1 mr-[26px]"></i>
						<span class="text-[10px] uppercase font-extrabold"><?php echo esc_html__('All Categories', 'botanica'); ?></span>
					</div>
				</div>
				<div id="desktop_menu">
