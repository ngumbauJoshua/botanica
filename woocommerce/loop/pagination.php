...
<nav class="woocommerce-pagination rbb-woo-pagination text-center" data-type="<?php echo esc_attr($pagination_type); ?>">
	...
	if ( in_array($pagination_type, [ 'load_more', 'infinity' ], true) ) {
		...
		<div class="pagination_count mb-[15px] pt-6 text-xs">
			...
			echo wp_kses_post('Showing <strong>' . $last . '</strong> of <strong>' . $result_count . '</strong> products');
			?>
		</div>
		...
		if ( $current < $total ) {
			?>
			<button
				class="text-center h-12 px-5 rounded-[4px] min-w-[250px] rbb-load-more duration-300 text-[color:var(--rbb-title-color)] hover:text-white bg-[#ebebeb] hover:bg-[color:var(--rbb-general-primary-color)] text-[10px] <?php echo ( 'infinity' === $pagination_type ) ? 'hidden' : ''; ?>"
